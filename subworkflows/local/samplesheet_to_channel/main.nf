workflow SAMPLESHEET_TO_CHANNEL {
    ...
    gff = reference.map { meta ->
        return meta.gff && !meta.gtf ? [meta + [decompress_gff: meta.gff.endsWith('.gz') ?: false] + [run_gffread: meta.fasta ?: false] + [run_hisat2: meta.splice_sites ? false : true], meta.gff] : null
    }
    ...
    transcript_fasta = reference.map { meta ->
        return meta.transcript_fasta ? [meta + [decompress_fasta: meta.fasta.endsWith('.gz') ?: false] + [run_hisat2: meta.hisat2_index ? false : true] + [run_kallisto: meta.kallisto_index ? false : true] + [run_rsem: meta.rsem_index ? false : true] + [run_salmon: meta.salmon_index ? false : true] + [run_star: meta.star_index ? false : true], meta.transcript_fasta] : null
    }

workflow ASSET_TO_CHANNEL {
    ...
    intervals_bed = asset.map { meta, _fasta ->
        return meta.intervals_bed ? [meta.subMap(['genome', 'id', 'source', 'source_vcf', 'source_version', 'species']), meta.intervals_bed] : null
    }
    ...
    gff = asset.map { meta, fasta_ ->
        return meta.gff && !meta.gtf ? [meta.subMap(['genome', 'id', 'source', 'source_vcf', 'source_version', 'species']) + [decompress_gff: meta.gff.endsWith('.gz') ?: false] + [run_gffread: fasta_ ?: false] + [run_hisat2: meta.splice_sites ? false : true], meta.gff] : null
    }

    // If ends with .gz, decompress it
    // If any of the asset exists, then adding run_tools to false and skip the asset creation from the annotation derived file (gff, gtf or transcript_fasta)
    gtf = asset.map { meta, _fasta ->
        return meta.gtf ? [meta.subMap(['genome', 'id', 'source', 'source_vcf', 'source_version', 'species']) + [decompress_gtf: meta.gtf.endsWith('.gz') ?: false] + [run_hisat2: meta.splice_sites ? false : true], meta.gtf] : null
    }

    splice_sites = asset.map { meta, _fasta ->
        return meta.splice_sites ? [meta.subMap(['genome', 'id', 'source', 'source_vcf', 'source_version', 'species']), meta.splice_sites] : null
    }

    // If any of the asset exists, then adding run_tools to false and skip the asset creation from the annotation derived file (gff, gtf or transcript_fasta)
    transcript_fasta = asset.map { meta, _fasta ->
        return meta.transcript_fasta ? [meta.subMap(['genome', 'id', 'source', 'source_vcf', 'source_version', 'species']) + [run_hisat2: meta.hisat2_index ? false : true] + [run_kallisto: meta.kallisto_index ? false : true] + [run_rsem: meta.rsem_index ? false : true] + [run_salmon: meta.salmon_index ? false : true] + [run_star: meta.star_index ? false : true], meta.transcript_fasta] : null
    }
    ...
    vcf = asset
        .map { meta, _fasta ->
            return meta.vcf && !meta.vcf_tbi ? [meta.subMap(['genome', 'id', 'source', 'source_vcf', 'source_version', 'species']), file(meta.vcf)] : null
        }
        .transpose()
